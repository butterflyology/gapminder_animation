...
If you think that this is a fairly straight forward plot and doens't seem to contain a lot of information, you are correct. Right now we want to get the individual parts of the plot working and we will add on to it. This way, if something breaks the code, we'll know exactly where the bug was introduced. Eventually we will want to distinguish `Species` by color. But adding `color = Species` to the `ggplot` call will make the animation choppy, so we'll do that later and I'll show you a cleaver way to smooth it out . 
